...
func runShellAsync(ctx context.Context, cmdStr string, outputChanel chan string, debug bool) {
    ...
    err := cmd.Start()
    if err != nil {
        glog.Errorf("cmd start失败 %s, err: %+v", cmdStr, err)
        return
    }
    ...
    go func() {
        defer func() {
            if err := recover(); err != nil {
                glog.Errorf("cmd 失败 %s, err: %+v", cmdStr, err)
            }
            wg.Done()
        }()
        if debug {
            glog.Debugf("标准输出重定向到outputChanel,%s", cmdStr)
        }
        for readerOut.Scan() {
            line := readerOut.Text()
            outputChanel <- line
        ...
        if debug {
            glog.Debugf("标准输出重定向到outputChanel完成,%s", cmdStr)
        }
    }()
    go func() {
        defer func() {
        ...
        if debug {
            glog.Debugf("标准错误重定向到outputChanel,%s", cmd)
        }
        for readerErr.Scan() {
            line := readerErr.Text()
            outputChanel <- line
        ...
        if debug {
            glog.Debugf("标准错误重定向到outputChanel完成,%s", cmdStr)
        }
    }()
    if debug {
        glog.Debugf("标准输出，标准错误启动goroutine等待,%s", cmdStr)
    }
    _ = cmd.Wait()
    wg.Wait()
}
...
func IsDev(whiteUserName ...string) (r bool) {
    ...
    defer func() {
        glog.Debugf("当前用户：%s,IsDev:%t", current.Username, r)
    }()
